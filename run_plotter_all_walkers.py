...
# old trace plots
''' 
fig, ax = plt.subplots(nwalkers+0,2,figsize=(8,4*(nwalkers+0)))
f1vals=np.arange(0.01,1.0,0.01)
for walker in range(nwalkers):

  ax[walker,0].plot(pie_list[walker][:,0],'b.')
  ax[walker,1].plot(pie_list[walker][:,1],'r.')
  ax[walker,0].set_ylabel(r'$\pi_0$')
  ax[walker,1].set_ylabel(r'$\pi_1$')
...
fig, ax = plt.subplots(nwalkers+0,dj,figsize=(4*dj,4*(nwalkers+0)))
f1vals=np.arange(0.01,1.0,0.01)
for walker in range(nwalkers):

  for j in range(dj):
    ax[walker,j].plot(alphas_list[walker][:,0,j],'b.')
    ax[walker,j].plot(alphas_list[walker][:,1,j],'r.')
    ax[walker,j].set_ylabel(r'$\alpha_{k'+str(j)+'}$')
...
fig, ax = plt.subplots(nwalkers+0,db,figsize=(4*db,4*(nwalkers+0)))
f1vals=np.arange(0.01,1.0,0.01)
for walker in range(nwalkers):

  for b in range(db):
    ax[walker,b].plot(betas_list[walker][:,0,b],'b.')
    ax[walker,b].plot(betas_list[walker][:,1,b],'r.')
    ax[walker,b].set_ylabel(r'$\beta_{k'+str(b)+'}$')
    ax[-1,b].set_xlabel('Iterations')
fig.tight_layout()
...
ax[0].plot(np.arange(min(data[:,0]),max(data[:,0])+1.0,1.0),true_alphas[0],'black',label='true ttW')
ax[0].plot(np.arange(min(data[:,0]),max(data[:,0])+1.0,1.0),fake_alphas[0],'b--',label='Initial ttW')
ax[0].plot(np.arange(min(data[:,0]),max(data[:,0])+1.0,1.0),alpha_mean[0],'b-',label='Final ttW')
ax[0].fill_between(np.arange(min(data[:,0]),max(data[:,0])+1.0,1.0), alpha_mean[0]-alpha_err[0],alpha_mean[0]+alpha_err[0],alpha=0.2)
ax[0].legend(loc='upper right')

ax[1].plot(np.arange(min(data[:,0]),max(data[:,0])+1.0,1.0),true_alphas[1],'black',label='true 4-top')
ax[1].plot(np.arange(min(data[:,0]),max(data[:,0])+1.0,1.0),fake_alphas[1],'r--',label='Initial 4-top')
ax[1].plot(np.arange(min(data[:,0]),max(data[:,0])+1.0,1.0),alpha_mean[1],'r-',label='Final 4-top')
ax[1].fill_between(np.arange(min(data[:,0]),max(data[:,0])+1.0,1.0), alpha_mean[1]-alpha_err[1],alpha_mean[1]+alpha_err[1],alpha=0.2,color='red')
ax[1].legend(loc='upper right')

ax[2].plot(np.arange(min(data[:,1]),max(data[:,1])+1.0,1.0),true_betas[0],'black',label='true ttW')
ax[2].plot(np.arange(min(data[:,1]),max(data[:,1])+1.0,1.0),fake_betas[0],'b--',label='Initial ttW')
ax[2].plot(np.arange(min(data[:,1]),max(data[:,1])+1.0,1.0),beta_mean[0],'b-',label='Final ttW')
ax[2].fill_between(np.arange(min(data[:,1]),max(data[:,1])+1.0,1.0), beta_mean[0]-beta_err[0],beta_mean[0]+beta_err[0],alpha=0.2)
ax[2].legend(loc='upper right')

ax[3].plot(np.arange(min(data[:,1]),max(data[:,1])+1.0,1.0),true_betas[1],'black',label='true 4-top')
ax[3].plot(np.arange(min(data[:,1]),max(data[:,1])+1.0,1.0),fake_betas[1],'r--',label='Initial 4-top')
ax[3].plot(np.arange(min(data[:,1]),max(data[:,1])+1.0,1.0),beta_mean[1],'r-',label='Final 4-top')
ax[3].fill_between(np.arange(min(data[:,1]),max(data[:,1])+1.0,1.0), beta_mean[1]-beta_err[1],beta_mean[1]+beta_err[1],alpha=0.2,color='red')
ax[3].legend(loc='upper right')


ax[4].axvline(f1,color='black',label='true $\pi_1$')
ax[4].plot(f1vals,dirichlet.pdf([1-f1vals,f1vals],alpha=[1.0,1.0]),'r--',label='Initial $\pi_1$')
ax[4].hist(pie_list_all_walkers[:,1],color='red',label='Final 4-top',alpha=0.2,density=True)
...
ax[0,0].plot(np.arange(min(data[:,0]),max(data[:,0])+1.0,1.0),true_alphas[0],'black',label='true ttW')
ax[1,0].plot(np.arange(min(data[:,0]),max(data[:,0])+1.0,1.0),true_alphas[0],'black',label='true ttW')
ax[0,0].plot(np.arange(min(data[:,0]),max(data[:,0])+1.0,1.0),fake_alphas[0],'b--',label='Initial ttW')
ax[1,0].plot(np.arange(min(data[:,0]),max(data[:,0])+1.0,1.0),alpha_mean[0],'b-',label='Final ttW')
ax[0,0].fill_between(np.arange(min(data[:,0]),max(data[:,0])+1.0,1.0), fake_alphas[0]-np.sqrt(dirichlet.var(alpha=Nprior*fake_alphas[0])),fake_alphas[0]+np.sqrt(dirichlet.var(alpha=Nprior*fake_alphas[0])),alpha=0.2)
ax[1,0].fill_between(np.arange(min(data[:,0]),max(data[:,0])+1.0,1.0), alpha_mean[0]-alpha_err[0],alpha_mean[0]+alpha_err[0],alpha=0.2)
ax[0,0].legend(loc='upper right')
ax[1,0].legend(loc='upper right')

ax[0,1].plot(np.arange(min(data[:,0]),max(data[:,0])+1.0,1.0),true_alphas[1],'black',label='true 4-top')
ax[1,1].plot(np.arange(min(data[:,0]),max(data[:,0])+1.0,1.0),true_alphas[1],'black',label='true 4-top')
ax[0,1].plot(np.arange(min(data[:,0]),max(data[:,0])+1.0,1.0),fake_alphas[1],'r--',label='Initial 4-top')
ax[1,1].plot(np.arange(min(data[:,0]),max(data[:,0])+1.0,1.0),alpha_mean[1],'r-',label='Final 4-top')
ax[0,1].fill_between(np.arange(min(data[:,0]),max(data[:,0])+1.0,1.0), fake_alphas[1]-np.sqrt(dirichlet.var(alpha=Nprior*fake_alphas[1])),fake_alphas[1]+np.sqrt(dirichlet.var(alpha=Nprior*fake_alphas[1])),alpha=0.2,color='red')
ax[1,1].fill_between(np.arange(min(data[:,0]),max(data[:,0])+1.0,1.0), alpha_mean[1]-alpha_err[1],alpha_mean[1]+alpha_err[1],alpha=0.2,color='red')
ax[0,1].legend(loc='upper right')
ax[1,1].legend(loc='upper right')

ax[0,2].plot(np.arange(min(data[:,1]),max(data[:,1])+1.0,1.0),true_betas[0],'black',label='true ttW')
ax[1,2].plot(np.arange(min(data[:,1]),max(data[:,1])+1.0,1.0),true_betas[0],'black',label='true ttW')
ax[0,2].plot(np.arange(min(data[:,1]),max(data[:,1])+1.0,1.0),fake_betas[0],'b--',label='Initial ttW')
ax[1,2].plot(np.arange(min(data[:,1]),max(data[:,1])+1.0,1.0),beta_mean[0],'b-',label='Final ttW')
ax[0,2].fill_between(np.arange(min(data[:,1]),max(data[:,1])+1.0,1.0), fake_betas[0]-np.sqrt(dirichlet.var(alpha=Nprior*fake_betas[0])),fake_betas[0]+np.sqrt(dirichlet.var(alpha=Nprior*fake_betas[0])),alpha=0.2)
ax[0,2].legend(loc='upper right')
ax[1,2].fill_between(np.arange(min(data[:,1]),max(data[:,1])+1.0,1.0), beta_mean[0]-beta_err[0],beta_mean[0]+beta_err[0],alpha=0.2)
ax[1,2].legend(loc='upper right')

ax[0,3].plot(np.arange(min(data[:,1]),max(data[:,1])+1.0,1.0),true_betas[1],'black',label='true 4-top')
ax[1,3].plot(np.arange(min(data[:,1]),max(data[:,1])+1.0,1.0),true_betas[1],'black',label='true 4-top')
ax[0,3].plot(np.arange(min(data[:,1]),max(data[:,1])+1.0,1.0),fake_betas[1],'r--',label='Initial 4-top')
ax[1,3].plot(np.arange(min(data[:,1]),max(data[:,1])+1.0,1.0),beta_mean[1],'r-',label='Final 4-top')
ax[0,3].fill_between(np.arange(min(data[:,1]),max(data[:,1])+1.0,1.0), fake_betas[1]-np.sqrt(dirichlet.var(alpha=Nprior*fake_betas[1])),fake_betas[1]+np.sqrt(dirichlet.var(alpha=Nprior*fake_betas[1])),alpha=0.2,color='red')
ax[0,3].legend(loc='upper right')
ax[1,3].fill_between(np.arange(min(data[:,1]),max(data[:,1])+1.0,1.0), beta_mean[1]-beta_err[1],beta_mean[1]+beta_err[1],alpha=0.2,color='red')
ax[1,3].legend(loc='upper right')


ax[0,4].axvline(f1,color='black',label='true $\pi_1$')
ax[1,4].axvline(f1,color='black',label='true $\pi_1$')
ax[0,4].plot(f1vals,dirichlet.pdf([1-f1vals,f1vals],alpha=[1.0,1.0]),'r--',label='Initial $\pi_1$')
ax[1,4].hist(pie_list_all_walkers[:,1],color='red',label='Final 4-top',alpha=0.2,density=True)
#ax[4].fill_between(np.arange(min(data[:,1]),max(data[:,1])+1.0,1.0), beta_mean[1]-beta_err[1],beta_mean[1]+beta_err[1],alpha=0.2,color='red')
